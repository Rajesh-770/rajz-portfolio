...
export default function Experience() {
  return (
    <section
      id="experience"
      className="space-y-6 scroll-mt-28" // scroll-mt so navbar doesn’t cover it
    >
      <h2 className="text-2xl md:text-3xl font-semibold neon-text">
        Experience
      </h2>
      ...
      <div className="space-y-6">
        {experiences.map((exp) => (
          <motion.div
            ...
            // 👇 glow-hover is here – this card will glow on hover
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {exp.skills.map((skill) => (
                <span
                  key={skill}
                  className="px-3 py-1 rounded-full text-xs bg-white/5 border border-white/15 text-gray-200 backdrop-blur-sm glow-pill"
                >
                  {skill}
                </span>
              ))}
            ...
            <ul className="space-y-1 text-sm text-gray-300">
              {exp.points.map((p, idx) => (
                <li key={idx} className="flex gap-2">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-neonRed glow-dot shrink-0" />
                  <span>{p}</span>
                </li>
              ))}
            </ul>
